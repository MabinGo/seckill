...
public class SecKillQueryServiceTest {
  ...
  public void mutiQuerySuccess() throws Exception {
    ...
    for (int i = 0; i < totalSecKillSuccessCount; i++) {
      events.add(new PromotionEvent(PromotionEventType.SecKill, String.valueOf(i), 1, (float) 0.7));
    }

    //add a finish seckill history
    PromotionEvent start = new PromotionEvent(PromotionEventType.Start, totalSecKillSuccessCount, (float) 0.7);
    events.add(start);
    events.add(new PromotionEvent(PromotionEventType.Finish, start.getId(), totalSecKillSuccessCount, (float) 0.7));

    //add current seckill
    PromotionEvent current = new PromotionEvent(PromotionEventType.Start, totalSecKillSuccessCount, (float) 0.8);
    events.add(current);
